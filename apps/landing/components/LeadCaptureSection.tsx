...
export function LeadCaptureSection() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const parsed = leadSchema.safeParse({ email, source: leadSource });
    ...
    setState({ kind: "submitting", message: null });
      ...
      <div className="grid gap-10 rounded-[2.5rem] border border-white/6 bg-[var(--landing-panel)] px-8 py-10 shadow-[0_25px_80px_rgba(0,0,0,0.28)] lg:grid-cols-[1.05fr_0.95fr] lg:px-12 lg:py-14">
        <div className="max-w-2xl">
          ...
          <p className="mt-8 text-lg leading-8 text-zinc-400">
            Receive measured product updates, release notes, and early access
            notices for the options aggregation workflow.
          </p>
          <p className="mt-4 text-lg leading-8 text-zinc-400">
            We only use this channel for platform updates and onboarding into
            the Oggregator funnel.
          </p>
        </div>
        ...
        <div className="rounded-[2rem] border border-white/6 bg-[var(--landing-panel-strong)] p-6">
          <form className="flex h-full flex-col gap-5" onSubmit={onSubmit}>
            ...
            <p
              ...
              aria-live="polite"
            >
              {state.message ?? "Desk-grade product updates, routed with restraint."}
            </p>
          </form>
        </div>
      </div>
