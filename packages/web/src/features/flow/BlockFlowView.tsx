...
function BlockTradeRow({ trade, isExpanded, onToggle }: BlockTradeRowProps) {
  ...
  return (
    <div className={styles.tradeWrap}>
      <button className={styles.trade} data-side={trade.direction} data-whale={isWhale || undefined} onClick={onToggle}>
        <div className={styles.tradeMain}>
          ...
          <div className={styles.tradeInfo}>
            <div className={styles.tradeTop}>
              ...
              <span className={styles.side} data-side={trade.direction}>{trade.direction.toUpperCase()}</span>
            </div>
            <div className={styles.tradeBottom}>
              {legInfo ? (
                <>
                  ...
                  {!isMultiLeg ? <span className={styles.optType} data-type={legInfo.type}>{legInfo.type}</span> : null}
                </>
              ) : null}
            </div>
          </div>
          ...
          <div className={styles.tradeRight}>
            <span className={styles.notional} data-whale={isWhale || undefined}>{fmtUsdCompact(notionalUsd ?? premiumUsd)}</span>
          ...
          {isMultiLeg ? <span className={styles.chevron} data-expanded={isExpanded}>›</span> : null}
        </div>
      </button>
          ...
          {trade.legs.map((leg, index) => {
            ...
            return (
              <div key={index} className={styles.leg} data-side={leg.direction}>
                <span className={styles.legDir} data-side={leg.direction}>{leg.direction === "buy" ? "BUY" : "SELL"}</span>
                <span className={styles.legSize}>{leg.size} ct{leg.ratio > 1 ? ` · ratio ${leg.ratio}` : ""}</span>
...
export default function BlockFlowView({
  ...
  const historySummary = useBlockFlowHistorySummary(underlying, activeVenues, historyRange, scope === "history" && isHistoryRangeReady);
  const historyBounds = useBlockFlowHistorySummary(underlying, activeVenues, { start: null, end: null }, scope === "history");
  ...
  return (
    <>
      ...
      {isHistoryLoading ? (
        ...
        <EmptyState title={scope === "history" ? "No stored institutional trades in range" : "No institutional trades yet"} detail={scope === "history" ? "Try a wider date window, another venue selection, or let ingest collect more history." : "RFQ and block trades will appear here as they execute across Deribit, OKX, Bybit, Binance, and Derive."} />
      ) : (
        <div className={styles.list}>
          {trades.map((trade) => (
            <BlockTradeRow key={trade.tradeUid} trade={trade} isExpanded={expanded.has(trade.tradeUid)} onToggle={() => toggleTrade(trade.tradeUid)} />
          ))}
        </div>
      )}
    </>
