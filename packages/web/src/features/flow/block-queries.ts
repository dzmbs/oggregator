...
export function useBlockFlowHistorySummary(underlying: string, venues: string[], range: HistoryRange, enabled = true) {
  const query = useMemo(() => buildHistoryParams({ underlying, venues, range }), [range, underlying, venues]);
...
function buildHistoryParams(query: BlockHistoryPageQuery | { underlying: string; venues: string[]; range: HistoryRange }): URLSearchParams {
